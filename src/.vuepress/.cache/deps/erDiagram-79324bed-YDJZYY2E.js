...
// node_modules/.pnpm/mermaid@10.2.2/node_modules/mermaid/dist/erDiagram-79324bed.js
var ct = function() {
  var t = function(M, a, l, d) {
    for (l = l || {}, d = M.length; d--; l[M[d]] = a)
      ;
    return l;
  }, r = [1, 2], e = [1, 5], n = [6, 9, 11, 23, 25, 27, 29, 30, 31, 52], s = [1, 17], p = [1, 18], f = [1, 19], o = [1, 20], c = [1, 21], _ = [1, 22], E = [1, 25], g = [1, 30], O = [1, 31], y = [1, 32], A2 = [1, 33], Y = [1, 34], U = [6, 9, 11, 15, 20, 23, 25, 27, 29, 30, 31, 44, 45, 46, 47, 48, 52], X = [1, 46], I = [30, 31, 49, 50], N = [4, 6, 9, 11, 23, 25, 27, 29, 30, 31, 52], v = [44, 45, 46, 47, 48], k = [22, 37], x = [1, 66], P = [1, 65], Z = [22, 37, 39, 41], w = {
    trace: function() {
    },
    yy: {},
...
Expecting ` + tt.join(", ") + ", got '" + (this.terminals_[D] || D) + "'" : nt = "Parse error on line " + (J + 1) + ": Unexpected " + (D == dt ? "end of input" : "'" + (this.terminals_[D] || D) + "'"), this.parseError(nt, {
    ...
    var M = {
      ...
      setInput: function(a, l) {
        return this.yy = l || this.yy || {}, this._input = a, this._more = this._backtrack = this.done = false, this.yylineno = this.yyleng = 0, this.yytext = this.matched = this.match = "", this.conditionStack = ["INITIAL"], this.yylloc = {
          first_line: 1,
          first_column: 0,
          last_line: 1,
          last_column: 0
...
` + l + "^";
      ...
      next: function() {
        ...
        for (var m = this._currentRules(), i = 0; i < m.length; i++)
          if (d = this._input.match(this.rules[m[i]]), d && (!l || d[0].length > l[0].length)) {
            if (l = d, u = i, this.options.backtrack_lexer) {
              if (a = this.test_match(d, m[i]), a !== false)
                return a;
              if (this._backtrack) {
                l = false;
                continue;
        ...
        return l ? (a = this.test_match(l, m[u]), a !== false ? a : false) : this._input === "" ? this.EOF : this.parseError("Lexical error on line " + (this.yylineno + 1) + `. Unrecognized text.
` + this.showPosition(), {
          text: "",
          token: null,
          line: this.yylineno
  ...
  return S3.prototype = w, w.Parser = S3, new S3();
}();
ct.parser = ct;
var Ct = ct;
var j = {};
...
var Zt = function(t, r, e) {
  de.parseDirective(this, t, r, e);
};
var pt = function(t) {
  return j[t] === void 0 && (j[t] = { attributes: [] }, S2.info("Added new entity :", t)), j[t];
};
var Ft = () => j;
var Wt = function(t, r) {
  let e = pt(t), n;
  for (n = r.length - 1; n >= 0; n--)
    e.attributes.push(r[n]), S2.debug("Added attribute ", r[n].attributeName);
};
var Vt = function(t, r, e, n) {
  let s = {
    entityA: t,
    roleA: r,
    entityB: e,
  ...
  lt.push(s), S2.debug("Added new relationship :", s);
};
var Ut2 = () => lt;
var zt = function() {
  j = {}, lt = [], Fa();
...
var Jt = "6ba7b810-9dad-11d1-80b4-00c04fd430c8";
...
function re(t) {
  ...
  for (let f = 0; f < s; ++f) {
    ...
    for (let y = 0; y < 80; ++y) {
      const A2 = Math.floor(y / 20), Y = ot(c, 5) + ee(A2, _, E, g) + O + r[A2] + o[y] >>> 0;
...
var oe = (t, r, e) => {
  ...
  let c = false, _ = false, E = 0, g = 0, O = 0, y = 0, A2 = f.height + n * 2, Y = 1;
  e.forEach((N) => {
    N.attributeKeyTypeList !== void 0 && N.attributeKeyTypeList.length > 0 && (c = true), N.attributeComment !== void 0 && (_ = true);
  }), e.forEach((N) => {
    const v = `${r.node().id}-attr-${Y}`;
    let k = 0;
    const x = ad(N.attributeType), P = t.append("text").classed("er entityLabel", true).attr("id", `${v}-type`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ut().fontFamily).style("font-size", p + "px").text(x), Z = t.append("text").classed("er entityLabel", true).attr("id", `${v}-name`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ut().fontFamily).style("font-size", p + "px").text(N.attributeName), w = {};
    w.tn = P, w.nn = Z;
    const z = P.node().getBBox(), S3 = Z.node().getBBox();
    if (E = Math.max(E, z.width), g = Math.max(g, S3.width), k = Math.max(z.height, S3.height), c) {
      const M = N.attributeKeyTypeList !== void 0 ? N.attributeKeyTypeList.join(",") : "", a = t.append("text").classed("er entityLabel", true).attr("id", `${v}-key`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ut().fontFamily).style("font-size", p + "px").text(M);
      w.kn = a;
      const l = a.node().getBBox();
      O = Math.max(O, l.width), k = Math.max(k, l.height);
    }
    if (_) {
      const M = t.append("text").classed("er entityLabel", true).attr("id", `${v}-comment`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ut().fontFamily).style("font-size", p + "px").text(N.attributeComment || "");
      w.cn = M;
      const a = M.node().getBBox();
      y = Math.max(y, a.width), k = Math.max(k, a.height);
    }
    w.height = k, o.push(w), A2 += k + n * 2, Y += 1;
  });
  let U = 4;
  c && (U += 2), _ && (U += 2);
  const X = E + g + O + y, I = {
    ...
    height: e.length > 0 ? A2 : Math.max(R.minEntityHeight, f.height + R.entityPadding * 2)
  };
  if (e.length > 0) {
    const N = Math.max(
      0,
    ...
    o.forEach((x) => {
      ...
      const Z = t.insert("rect", "#" + x.tn.node().id).classed(`er ${k}`, true).attr("x", 0).attr("y", v).attr("width", E + s * 2 + N).attr("height", x.height + n * 2), w = parseFloat(Z.attr("x")) + parseFloat(Z.attr("width"));
      x.nn.attr(
        "transform",
        "translate(" + (w + s) + "," + P + ")"
      );
      const z = t.insert("rect", "#" + x.nn.node().id).classed(`er ${k}`, true).attr("x", w).attr("y", v).attr("width", g + s * 2 + N).attr("height", x.height + n * 2);
      let S3 = parseFloat(z.attr("x")) + parseFloat(z.attr("width"));
      if (c) {
        x.kn.attr(
          "transform",
          "translate(" + (S3 + s) + "," + P + ")"
        );
        const M = t.insert("rect", "#" + x.kn.node().id).classed(`er ${k}`, true).attr("x", S3).attr("y", v).attr("width", O + s * 2 + N).attr("height", x.height + n * 2);
        S3 = parseFloat(M.attr("x")) + parseFloat(M.attr("width"));
      }
      _ && (x.cn.attr(
        "transform",
        "translate(" + (S3 + s) + "," + P + ")"
      ), t.insert("rect", "#" + x.cn.node().id).classed(`er ${k}`, "true").attr("x", S3).attr("y", v).attr("width", y + s * 2 + N).attr("height", x.height + n * 2)), v += x.height + n * 2, k = k === "attributeBoxOdd" ? "attributeBoxEven" : "attributeBoxOdd";
    });
  } else
    I.height = Math.max(R.minEntityHeight, A2), r.attr("transform", "translate(" + I.width / 2 + "," + I.height / 2 + ")");
  return I;
};
var ce = function(t, r, e) {
  ...
  return n.forEach(function(p) {
    ...
    s = s === void 0 ? f : s;
    const c = "text-" + f, _ = o.append("text").classed("er entityLabel", true).attr("id", c).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "middle").style("font-family", Ut().fontFamily).style("font-size", R.fontSize + "px").text(p), { width: E, height: g } = oe(
      o,
      _,
      r[p].attributes
    ), y = o.insert("rect", "#" + c).classed("er entityBox", true).attr("x", 0).attr("y", 0).attr("width", E).attr("height", g).node().getBBox();
    e.setNode(f, {
      width: y.width,
      height: y.height,
      shape: "rect",
...
var de2 = function(t, r, e, n, s) {
  ...
  const p = e.edge(
    ...
    return A2.y;
  }).curve(mf), o = t.insert("path", "#" + n).classed("er relationshipLine", true).attr("d", f(p.points)).style("stroke", R.stroke).style("fill", "none");
  ...
  const _ = o.node().getTotalLength(), E = o.node().getPointAtLength(_ * 0.5), g = "rel" + ft, y = t.append("text").classed("er relationshipLabel", true).attr("id", g).attr("x", E.x).attr("y", E.y).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-family", Ut().fontFamily).style("font-size", R.fontSize + "px").text(r.roleA).node().getBBox();
  t.insert("rect", "#" + g).classed("er relationshipLabelBox", true).attr("x", E.x - y.width / 2).attr("y", E.y - y.height / 2).attr("width", y.width).attr("height", y.height);
};
var ue = function(t, r, e, n) {
  R = Ut().er, S2.info("Drawing ER diagram");
  const s = Ut().securityLevel;
  let p;
  s === "sandbox" && (p = At("#i" + r));
  const o = (s === "sandbox" ? At(p.nodes()[0].contentDocument.body) : At("body")).select(`[id='${r}']`);
  C.insertMarkers(o, R);
  let c;
  c = new A({
    multigraph: true,
  ...
  const O = o.node().getBBox(), y = O.width + g * 2, A2 = O.height + g * 2;
  Hg(o, A2, y, R.useMaxWidth), o.attr("viewBox", `${O.x - g} ${O.y - g} ${y} ${A2}`);
};
var fe = "28e9f9db-3c8d-5aa5-9faf-44286ae5937c";
function ye(t = "", r = "") {
...
//# sourceMappingURL=erDiagram-79324bed-YDJZYY2E.js.map
