...
// node_modules/.pnpm/mermaid@10.2.2/node_modules/mermaid/dist/pieDiagram-9f52f3c8.js
...
function Ot() {
  var t3 = Pt, s = Vt, c = null, o = E(0), y = E(Fx), u = E(0);
  function h(l) {
    var g, m = (l = t(l)).length, D, C, x = 0, $ = new Array(m), w = new Array(m), A = +o.apply(this, arguments), N = Math.min(Fx, Math.max(-Fx, y.apply(this, arguments) - A)), W, I = Math.min(Math.abs(N) / m, u.apply(this, arguments)), V = I * (N < 0 ? -1 : 1), v;
    for (g = 0; g < m; ++g)
...
var rt = function() {
  var t3 = function(k, e, i, a) {
    ...
    parse: function(e) {
      var i = this, a = [0], n = [], f = [null], r = [], p = this.table, d = "", F = 0, Q = 0, K = 2, q = 1, ft = r.slice.call(arguments, 1), _ = Object.create(this.lexer), M = { yy: {} };
      ...
      function yt() {
        var O;
        return O = n.pop() || _.lex() || q, typeof O != "number" && (O instanceof Array && (n = O, O = n.pop()), O = i.symbols_[O] || O), O;
      }
      for (var b, j, S2, tt, R = {}, G, P, at, H; ; ) {
        if (j = a[a.length - 1], this.defaultActions[j] ? S2 = this.defaultActions[j] : ((b === null || typeof b > "u") && (b = yt()), S2 = p[j] && p[j][b]), typeof S2 > "u" || !S2.length || !S2[0]) {
          var et = "";
          H = [];
          for (G in p[j])
...
` + _.showPosition() + `
Expecting ` + H.join(", ") + ", got '" + (this.terminals_[b] || b) + "'" : et = "Parse error on line " + (F + 1) + ": Unexpected " + (b == q ? "end of input" : "'" + (this.terminals_[b] || b) + "'"), this.parseError(et, {
            text: _.match,
        ...
        if (S2[0] instanceof Array && S2.length > 1)
          throw new Error("Parse Error: multiple actions possible at state: " + j + ", token: " + b);
        switch (S2[0]) {
          case 1:
            a.push(b), f.push(_.yytext), r.push(_.yylloc), a.push(S2[1]), b = null, Q = _.yyleng, d = _.yytext, F = _.yylineno, Z = _.yylloc;
            break;
          case 2:
            if (P = this.productions_[S2[1]][1], R.$ = f[f.length - P], R._$ = {
              ...
              return tt;
            P && (a = a.slice(0, -1 * P * 2), f = f.slice(0, -1 * P), r = r.slice(0, -1 * P)), a.push(this.productions_[S2[1]][0]), f.push(R.$), r.push(R._$), at = p[a[a.length - 2]][a[a.length - 1]], a.push(at);
            break;
          case 3:
            return true;
        }
...
var Nt = function(t3, s, c) {
  de.parseDirective(this, t3, s, c);
};
var Wt = function(t3, s) {
  t3 = Fn.sanitizeText(t3, Ut()), J[t3] === void 0 && (J[t3] = s, S.debug("Added new section :", t3));
};
var Ct = () => J;
var Ft = function(t3) {
  st = t3;
...
` + t3);
    const v = Ut().securityLevel;
    let E2;
    v === "sandbox" && (E2 = At("#i" + s));
    const k = v === "sandbox" ? At(E2.nodes()[0].contentDocument.body) : At("body"), e = v === "sandbox" ? E2.nodes()[0].contentDocument : document;
    o.db.clear(), o.parser.parse(t3), S.debug("Parsed info diagram");
    ...
    const a = k.select("#" + s);
    Hg(a, B, L, T.pie.useMaxWidth), i.setAttribute("viewBox", "0 0 " + L + " " + B);
    ...
    let [r] = Nn(n.pieOuterStrokeWidth);
    r ?? (r = 2);
    ...
    var I = g.selectAll(".legend").data(x.domain()).enter().append("g").attr("class", "legend").attr("transform", function(p, d) {
      const F = u + h, Q = F * x.domain().length / 2, K = 12 * u, q = d * F - Q;
      return "translate(" + K + "," + q + ")";
    });
    ...
    S.error("Error while rendering info diagram"), S.error(v);
  }
};
var Qt = {
  draw: Ut2
...
//# sourceMappingURL=pieDiagram-9f52f3c8-XM2TGCUQ.js.map
